...
def load_calibrate_timedomain(path, offset_path):
    ...
    def apply_calibration(csi, pos, time):
        sto_offset = tf.tensordot(
            ...
            2
            * np.pi
            * tf.range(tf.shape(csi)[1], dtype=np.float32)
            / tf.cast(tf.shape(csi)[1], np.float32),
            axes=0,
        )
        cpo_offset = tf.tensordot(
            tf.constant(offsets["cpo"]),
            tf.ones(tf.shape(csi)[1], dtype=np.float32),
            axes=0,
        )
    ...
    def csi_time_domain(csi, pos, time):
        csi = tf.signal.fftshift(
            tf.signal.ifft(tf.signal.fftshift(csi, axes=1)), axes=1
        )
    ...
    def order_by_antenna_assignments(csi, pos, time):
        csi = tf.stack(
            [tf.gather(csi, antenna_indices) for antenna_indices in ASSIGNMENTS]
        )
        return csi, pos, time
    ...
    dataset = dataset.map(
        order_by_antenna_assignments, num_parallel_calls=tf.data.AUTOTUNE
    )
